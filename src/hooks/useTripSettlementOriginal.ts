...
const getOriginalSpent = (participant: Participant): number =>
  typeof participant.totalOriginalSpent === "number"
    ? participant.totalOriginalSpent
    : 0;

export const useTripSettlementOriginal = (
  ...
  const averagePerPerson =
    participantCount > 0 ? totalOriginalExpense / participantCount : 0;

  const mainSpender =
    participants.length > 0
      ? participants.reduce(
          (max, p) => (getOriginalSpent(p) > getOriginalSpent(max) ? p : max),
          participants[0],
        )
      : null;
  ...
  const getParticipantBalance = (participantId: string): number => {
    ...
    return getOriginalSpent(participant) - averagePerPerson;
  };
  ...
  const calculateSettlements = (): Settlement[] => {
    ...
    const balances = participants.map((p) => ({
      participant: p,
      balance: getOriginalSpent(p) - averagePerPerson,
    }));
